#!/usr/bin/env perl
# $Id: eval07.pl,v 1.4 2007/04/02 21:17:34 dyuret Exp $
...
my ($usage) = <<EOT
  ...
  Both files should be in UTF-8 encoded CoNLL-07 tabular format.
...
if (defined $opt_v)
{
    my $id = '$Id: eval07.pl,v 1.4 2007/04/02 21:17:34 dyuret Exp $';
    my @parts = split ' ',$id;
...
printf OUT "  %-10s | %-5s | %-5s |      |of err| %-5s |      |of err| %-5s |\n",

  'Rate', ' ', 'err', ' err', 'wrong' ;

printf OUT "%s\n", "  -----------+-------+-------+------+------+-------+------+------+-------+-------" ;

printf OUT "  %-10s | %5d | %5d | %3.0f%% | %3.0f%% | %5d | %3.0f%% | %3.0f%% | %5d | %3.0f%%\n",
  'total', $counts{tot},
  $counts{err_head}{tot}, $counts{err_head}{tot}*100.0/$counts{tot}, 100,
  $counts{err_dep}{tot}, $counts{err_dep}{tot}*100.0/$counts{tot}, 100,
...
### marie candito : proportion of errors						     
				       $counts{pos}{$pos}{err_head}{tot}*100.0/$counts{err_head}{tot},
    $counts{pos}{$pos}{err_dep}{tot}, $counts{pos}{$pos}{err_dep}{tot}*100.0/$counts{pos}{$pos}{tot},
### marie candito : proportion of errors						     
				       $counts{pos}{$pos}{err_dep}{tot}*100.0/$counts{err_dep}{tot},
